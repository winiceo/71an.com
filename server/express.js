...
module.exports = (backend, options, cb) => {
  let server = express();
  ...
  server.use(express.static(path.join(root, "public"), maxCacheAge));

  console.log(path.join(root, "public"))
  server.use("/node_modules", express.static(path.join(root, server.locals.node_path),
    maxCacheAge));
  ...
  server.use("/resources/remix", express.static(path.join(root,
    "public/resources/remix"), maxCacheAge));
  ...
  //     excludeLocaleInUrl: ["/api/*"]
  // }));


  server.use(backend.modelMiddleware())

  /**
   * API routes
   */
  routes.init(server, middleware, backend);
